 ...
 */
class fpGridSphere : public fxFitpackFitter {
public:
    // ===========================================================================================
     ...
     */
    fpGridSphere() : fxFitpackFitter(NoAlloc{}) {
     ...
     */
    fpGridSphere(const fpGridSphere& other) : fxFitpackFitter(NoAlloc{}) {
     ...
     */
    fpGridSphere(fpGridSphere&& other) noexcept : fxFitpackFitter(NoAlloc{}) {
        *as<fitpack_grid_sphere_c>() = fitpack_grid_sphere_c_null;
     ...
     */
    explicit fpGridSphere(fitpack_grid_sphere_c& c_wrapper, bool move = false) : fxFitpackFitter(NoAlloc{}) {
        if (move) {
     ...
     */
    explicit fpGridSphere(fitpack_grid_sphere_c& c_wrapper, ViewTag) : fxFitpackFitter(NoAlloc{}) {
     ...
     * @brief new_fit
     */
    virtual int32_t new_fit(std::vector<double>& u, std::vector<double>& v, int32_t z_n1, int32_t z_n2, std::vector<double>& z, double* smoothing = nullptr) const {
        int32_t n = static_cast<int32_t>(u.size());
...
protected:
    explicit fpGridSphere(NoAlloc tag) : fxFitpackFitter(tag) {}
