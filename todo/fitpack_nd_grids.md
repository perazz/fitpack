...
| **C bindings (slice 10)** | `regrid_c` / `bispev_c` template | **DONE** — `src/fitpack_gridded_splines_c.f90`, `include/fitpack_gridded_spline_c.h`, `include/fpGridSpline.hpp`, generated from `fitpack_bindings.yaml` with the rest of the family. Assumed-rank `z(..)` uses the flat entry path with an explicit `m`; the `cross_section` / `derivative_spline` derived returns are bound; `fpPoint<dims>` overloads of `eval`/`dfdx`/`integral` come from `extra_methods/fpGridSplinePoints.hpp`. Fixed-size `MAX_IDIM` metadata component accessors remain a known generator gap. |
