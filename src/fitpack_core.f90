...
module fitpack_core
    ...
    contains
      ...
      recursive subroutine bispeu(tx,nx,ty,ny,c,kx,ky,x,y,z,m,wrk,lwrk,ier)
 ...
 100  return
      ...
      nmin = 2*k1
      if(m<k1 .or. nest<nmin) go to 50
      lwest = m*k1+nest*(7+3*k)
      if(lwrk<lwest) go to 50
      if(xb>x(1) .or. xe<x(m)) go to 50
      do 10 i=2,m
         if(x(i-1)>x(i)) go to 50
  10  continue
      if(iopt>=0) go to 30
 ...
 100  return
      ...
      !  ..array arguments..
      real(RKIND) t(n),c(nc),cp(np),cc(nc),t1(n),t2(n)
      !  ..local scalars..
 ...
 440  return
      ...
      !  ..array arguments..
      real(RKIND) x(m),y(m),w(m),t(n),e(n),c(n),sx(m),a(n,4),b(nm,maxbin), &
 ...
 110  call fpgrsp(ifsu,ifsv,ifbu,ifbv,0,u,mu,v,mv,r,mr,dr, &
      ...
      !  ..array arguments..
      real(RKIND) x(m),y(m),tx(nx),ty(ny)
 ...
 990  return
      ...
      real(RKIND) tol,sq
      !  ..array arguments..
      real(RKIND) a(na,m),f(n),c(n),aa(n,m),ff(n),h(m)
      !  ..local scalars..
