...
!
! **************************************************************************************************
module fitpack_closed_curves_c
     ...
     public :: fitpack_closed_curve_c_get_pointer
     public :: fitpack_closed_curve_c_pointer
     ...
     public :: fitpack_closed_curve_c_interpolating
     public :: fitpack_closed_curve_c_eval_one
     public :: fitpack_closed_curve_c_smoothing
     ...
     type(fitpack_closed_curve_c), parameter, public :: &
        fitpack_closed_curve_c_null = fitpack_closed_curve_c(cptr=c_null_ptr)

     contains

     !> Get pointer to embedded Fortran object
     subroutine fitpack_closed_curve_c_get_pointer(this,fptr)
        type(fitpack_closed_curve_c), intent(in) :: this
        type(fitpack_closed_curve), pointer, intent(inout) :: fptr
     ...
     end subroutine fitpack_closed_curve_c_get_pointer

     !> Get embedded Fortran object; allocate it if not done yet
     subroutine fitpack_closed_curve_c_pointer(this,fthis)
        ...
        call fitpack_closed_curve_c_get_pointer(this,fthis)
        ...
        if (ierr/=0) stop 'fitpack_closed_curve_c_pointer allocation error'

        ! Locate new fortran object
        call fthis%destroy()
        this%cptr = c_loc(fthis)

     end subroutine fitpack_closed_curve_c_pointer

     !> Instantiate Fortran object
     subroutine fitpack_closed_curve_c_allocate(this) &
                bind(C,name='fitpack_closed_curve_c_allocate')
        type(fitpack_closed_curve_c), intent(inout) :: this
        type(fitpack_closed_curve), pointer :: fthis
        call fitpack_closed_curve_c_pointer(this,fthis)
     end subroutine fitpack_closed_curve_c_allocate

     !> Deallocate Fortran object
     subroutine fitpack_closed_curve_c_destroy(this) &
        ...
        if (associated(fthis)) then
            ...
            if (ierr/=0) stop 'fitpack_closed_curve_c_destroy deallocation error'
        end if
     ...
     subroutine fitpack_closed_curve_c_copy(this,that) &
                bind(C,name='fitpack_closed_curve_c_copy')
         type(fitpack_closed_curve_c), intent(inout) :: this,that

         type(fitpack_closed_curve), pointer :: fthis,fthat

         call fitpack_closed_curve_c_get_pointer(that,fthat)
         ...
         if (associated(fthat)) then
            ...
            call fitpack_closed_curve_c_destroy(this)
         end if

     end subroutine fitpack_closed_curve_c_copy

      !> Move allocation from one object to another
      subroutine fitpack_closed_curve_c_move_alloc(to,from) &
         ...
         call fitpack_closed_curve_c_destroy(to)
     ...
     subroutine fitpack_closed_curve_c_new_points(this,ndim,npts,x,y,w) &
                bind(c,name='fitpack_closed_curve_c_new_points')
         type(fitpack_closed_curve_c), intent(inout) :: this
         ...
         call fitpack_closed_curve_c_pointer(this, fcurve)

         call fcurve%new_points(x,y,w)

     end subroutine fitpack_closed_curve_c_new_points

     subroutine fitpack_closed_curve_c_set_default_parameters(this) &
         bind(c,name='fitpack_closed_curve_c_set_default_parameters')
         type(fitpack_closed_curve_c), intent(inout) :: this
         type(fitpack_closed_curve), pointer :: fcurve
     ...
     !> Wrapper to new_fit
     integer(FP_FLAG) function fitpack_closed_curve_c_new_fit(this,ndim,npts,x,u,w,smoothing,order) result(ierr) &
                                bind(c,name='fitpack_closed_curve_c_new_fit')
         type(fitpack_closed_curve_c), intent(inout) :: this
     ...
     integer(FP_FLAG) function fitpack_closed_curve_c_fit(this,smoothing,order) result(ierr) &
                               bind(c,name='fitpack_closed_curve_c_fit')
        type(fitpack_closed_curve_c), intent(inout) :: this
        real   (FP_REAL), optional, intent(in) :: smoothing
        integer(FP_SIZE), optional, intent(in) :: order
     ...
     integer(FP_FLAG) function fitpack_closed_curve_c_interpolating(this) result(ierr) &
                                bind(c,name='fitpack_closed_curve_c_interpolating')
        type(fitpack_closed_curve_c), intent(inout) :: this
     ...
     end function fitpack_closed_curve_c_interpolating

     !> Wrapper to curve_eval_one
     integer(FP_FLAG) function fitpack_closed_curve_c_eval_one(this,u,y) result(ierr) &
                            bind(c,name='fitpack_closed_curve_c_eval_one')
        type(fitpack_closed_curve_c), intent(inout) :: this
     ...
     end function fitpack_closed_curve_c_eval_one
...
!
     ...
     integer(FP_FLAG) function fitpack_closed_curve_c_derivative(this,u,order,ddx) result(ierr) &
                            bind(c,name='fitpack_closed_curve_c_derivative')
        type(fitpack_closed_curve_c), intent(inout) :: this
        real   (FP_REAL), intent(in), value :: u
     ...
     end function fitpack_closed_curve_c_derivative

     !> Get smoothing
     real(FP_REAL) function fitpack_closed_curve_c_smoothing(this) &
                            bind(c,name='fitpack_closed_curve_c_smoothing')
        type(fitpack_closed_curve_c), intent(inout) :: this
        ...
        if (associated(fcurve)) then
           fitpack_closed_curve_c_smoothing = fcurve%smoothing
        else
           fitpack_closed_curve_c_smoothing = -1.0_FP_REAL
        endif

     end function fitpack_closed_curve_c_smoothing


     !> Get dimensions
     integer(FP_SIZE) function fitpack_closed_curve_c_idim(this) &
                            bind(c,name='fitpack_closed_curve_c_idim')
        type(fitpack_closed_curve_c), intent(inout) :: this
        ...
        if (associated(fcurve)) then
           fitpack_closed_curve_c_idim = fcurve%idim
        else
           fitpack_closed_curve_c_idim = 0
        end if
     ...
     real(FP_REAL) function fitpack_closed_curve_c_mse(this) &
                            bind(c,name='fitpack_closed_curve_c_mse')
        type(fitpack_closed_curve_c), intent(inout) :: this
        ...
        if (associated(fcurve)) then
           fitpack_closed_curve_c_mse = fcurve%fp
        else
           fitpack_closed_curve_c_mse = 0.0_FP_REAL
        endif

     end function fitpack_closed_curve_c_mse

     !> Get spline degree
     integer(FP_SIZE) function fitpack_closed_curve_c_degree(this) &
                            bind(c,name='fitpack_closed_curve_c_degree')
        type(fitpack_closed_curve_c), intent(inout) :: this
        ...
        if (associated(fcurve)) then
           fitpack_closed_curve_c_degree = fcurve%order
        else
           fitpack_closed_curve_c_degree = 0
        end if

     end function fitpack_closed_curve_c_degree
!

end module fitpack_closed_curves_c
